...
export function ForgotPasswordScreen({ navigation }: Props) {
  ...
  const onSubmit = handleSubmit(async ({ phone }) => {
    ...
    try {
      ...
      setPhone(phone);
      navigation.navigate("OTP", { flow: "forgot" });
    } catch (e) {
    ...
    <SafeAreaView className="flex-1 bg-white dark:bg-slate-950" edges={["bottom"]}>
      ...
      >
        ...
        >
          ...
          {apiError ? <Text className="mt-2 text-center text-sm text-red-500">{apiError}</Text> : null}

          <View className="mt-6">
            <Button title="Gửi OTP" loading={loading} disabled={!isValid} onPress={onSubmit} />
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
